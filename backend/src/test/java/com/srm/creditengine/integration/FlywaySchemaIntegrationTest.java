...
class FlywaySchemaIntegrationTest {
    ...
    void shouldSeedSupportedCurrenciesAndReceivableTypes() {
        ...
        List<String> receivableTypes = jdbcTemplate.queryForList(
                "SELECT code FROM receivable_types ORDER BY code",
                String.class
        );
        BigDecimal mercantileSpread = jdbcTemplate.queryForObject(
                "SELECT monthly_spread FROM receivable_types WHERE code = 'MERCANTILE_DUPLICATE'",
                BigDecimal.class
        );
        BigDecimal postDatedSpread = jdbcTemplate.queryForObject(
    ...
    void shouldUseDecimalColumnsForFinancialData() {
        Integer floatingPointColumns = jdbcTemplate.queryForObject(
                """
                SELECT COUNT(*)
    ...
    void shouldCreateCriticalConstraintsAndIndexesForIntegrityAndReporting() {
        ...
        assertThat(receivablesDdl).contains("UNIQUE KEY `uk_receivables_assignor_external_reference` (`assignor_id`,`external_reference`)");
        assertThat(settlementItemsDdl).contains("UNIQUE KEY `uk_settlement_items_receivable` (`receivable_id`)");
        assertThat(exchangeRatesDdl).contains(
                "KEY `idx_exchange_rates_pair_valid_at` (`source_currency_code`,`target_currency_code`,`valid_at` DESC,`created_at` DESC)"
        );
        assertThat(settlementsDdl).contains("KEY `idx_settlements_statement_default` (`settled_at` DESC,`id`)");
        assertThat(settlementItemsDdl).contains("KEY `idx_settlement_items_receivable_type` (`receivable_type_code`)");
        assertThat(settlementItemsDdl).contains("KEY `idx_settlement_items_payment_currency` (`payment_currency_code`)");
    }
