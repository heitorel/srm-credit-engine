...
class SettlementStatementControllerIntegrationTest extends AbstractMySqlIntegrationTest {
  ...
  void shouldReturnDefaultFirstPage() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement"))
        ...
        .andExpect(
            jsonPath("$.content[0].settlementId").value("30000000-0000-0000-0000-000000000003"))
        .andExpect(
            jsonPath("$.content[1].settlementId").value("10000000-0000-0000-0000-000000000001"))
        .andExpect(
            jsonPath("$.content[2].settlementId").value("20000000-0000-0000-0000-000000000002"));
  }

  @Test
  void shouldReturnRequestedPageAndSize() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?page=1&size=1"))
        .andExpect(status().isOk())
        ...
        .andExpect(
            jsonPath("$.content[0].settlementId").value("10000000-0000-0000-0000-000000000001"));
  }

  @Test
  void shouldRejectNegativePage() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?page=-1"))
  ...
  void shouldRejectZeroSize() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?size=0"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.message").value("Invalid pagination parameters."))
  ...
  void shouldRejectPageSizeAboveMaximum() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?size=101"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.message").value("Invalid pagination parameters."))
        .andExpect(jsonPath("$.details[0].field").value("size"));
  }
  ...
  void shouldReturnAscendingSortWhenRequested() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?sort=settledAt,asc"))
        .andExpect(status().isOk())
        .andExpect(
            jsonPath("$.content[0].settlementId").value("10000000-0000-0000-0000-000000000001"))
        .andExpect(
            jsonPath("$.content[1].settlementId").value("20000000-0000-0000-0000-000000000002"))
        .andExpect(
            jsonPath("$.content[2].settlementId").value("30000000-0000-0000-0000-000000000003"));
  }

  @Test
  void shouldRejectUnsupportedSortField() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?sort=status,asc"))
        .andExpect(status().isBadRequest())
  ...
  void shouldFilterByDateRange() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?from=2026-07-08&to=2026-07-08"))
        .andExpect(status().isOk())
  ...
  void shouldRejectInvalidDateRange() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?from=2026-07-09&to=2026-07-08"))
        .andExpect(status().isBadRequest())
  ...
  void shouldFilterByAssignorId() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?assignorId=aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.totalElements").value(2))
        .andExpect(jsonPath("$.content", hasSize(2)));
  }
  ...
  void shouldFilterByAssignorDocument() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?assignorDocument=12345678000199"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.totalElements").value(2))
        .andExpect(jsonPath("$.content", hasSize(2)));
  }
  ...
  void shouldFilterByPaymentCurrency() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?paymentCurrency=USD"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.totalElements").value(2))
        .andExpect(jsonPath("$.content", hasSize(2)))
  ...
  void shouldFilterBySourceCurrency() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?sourceCurrency=USD"))
        .andExpect(status().isOk())
  ...
  void shouldFilterByReceivableType() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?receivableType=POST_DATED_CHECK"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.totalElements").value(1))
        .andExpect(
            jsonPath("$.content[0].settlementId").value("20000000-0000-0000-0000-000000000002"));
  }

  @Test
  void shouldFilterByStatus() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?status=CANCELLED"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.totalElements").value(1))
  ...
  void shouldCombineMultipleFilters() throws Exception {
    mockMvc
        .perform(
            get(
                "/api/settlements/statement?assignorDocument=12345678000199&paymentCurrency=USD&status=SETTLED"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.totalElements").value(1))
        .andExpect(
            jsonPath("$.content[0].settlementId").value("10000000-0000-0000-0000-000000000001"))
  ...
  void shouldReturnEmptyPageWhenNoRowsMatch() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?assignorDocument=00000000000000"))
        .andExpect(status().isOk())
  ...
  void shouldReturnStructuredErrorForUnsupportedCurrency() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?paymentCurrency=EUR"))
        .andExpect(status().isBadRequest())
  ...
  void shouldReturnStructuredErrorForUnsupportedReceivableType() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?receivableType=INVOICE"))
  ...
  void shouldReturnStructuredErrorForUnsupportedStatus() throws Exception {
    mockMvc
        .perform(get("/api/settlements/statement?status=ARCHIVED"))
        .andExpect(status().isUnprocessableEntity())
