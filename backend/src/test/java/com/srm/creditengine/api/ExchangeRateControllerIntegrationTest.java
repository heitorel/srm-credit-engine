...
class ExchangeRateControllerIntegrationTest extends AbstractMySqlIntegrationTest {
    ...
    void shouldCreateExchangeRate() throws Exception {
        mockMvc.perform(post("/api/exchange-rates")
                ...
                .andExpect(header().string("Location", org.hamcrest.Matchers.matchesPattern("/api/exchange-rates/.+")))
    ...
    void shouldRejectExchangeRateWithSameSourceAndTarget() throws Exception {
        mockMvc.perform(post("/api/exchange-rates")
                ...
                .andExpect(jsonPath("$.message").value("Source currency and target currency must be different."))
    ...
    void shouldFindLatestExchangeRateByExactPair() throws Exception {
        ...
        insertExchangeRate(rateId, "USD", "BRL", "5.25000000", "2026-07-07T13:00:00Z", "2026-07-07T13:05:00Z");
    ...
    void shouldReturnMostRecentValidExchangeRate() throws Exception {
        insertExchangeRate("11111111-1111-1111-1111-111111111111", "USD", "BRL", "5.10000000", "2026-07-07T13:00:00Z", "2026-07-07T13:05:00Z");
        insertExchangeRate("22222222-2222-2222-2222-222222222222", "USD", "BRL", "5.25000000", "2026-07-08T13:00:00Z", "2026-07-07T13:06:00Z");
    ...
    void shouldUseCreatedAtAsTieBreakerWhenValidAtIsEqual() throws Exception {
        insertExchangeRate("33333333-3333-3333-3333-333333333333", "USD", "BRL", "5.20000000", "2026-07-07T13:00:00Z", "2026-07-07T13:05:00Z");
        insertExchangeRate("44444444-4444-4444-4444-444444444444", "USD", "BRL", "5.30000000", "2026-07-07T13:00:00Z", "2026-07-07T13:06:00Z");
    ...
    void shouldNotFindInverseExchangeRateWhenOnlyOppositePairExists() throws Exception {
        ...
        mockMvc.perform(get("/api/exchange-rates/latest")
                ...
                .andExpect(jsonPath("$.message").value("No exchange rate found for currency pair BRL -> USD."));
    }
    ...
    void shouldReturnNotFoundWhenLatestExchangeRateDoesNotExist() throws Exception {
        mockMvc.perform(get("/api/exchange-rates/latest")
                        ...
                        .queryParam("targetCurrency", "BRL"))
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.message").value("No exchange rate found for currency pair USD -> BRL."));
    }
