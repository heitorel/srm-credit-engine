...
class SettlementStatementServiceTest {
    ...
    void shouldRejectInvalidStatementDateRangeBeforeQueryingRepository() {
        SettlementStatementService service = new SettlementStatementService(
                mock(ListReferenceDataService.class),
                mock(SettlementStatementQueryRepository.class)
        );

        assertThatThrownBy(() -> service.getStatement(new SettlementStatementRequest(
