...
public class SettlementStatementQueryRepository {
  ...
  public Page<SettlementStatementRow> findStatement(SettlementStatementFilter filter) {
    ...
    CriteriaQuery<SettlementStatementRowProjection> contentQuery =
        cb.createQuery(SettlementStatementRowProjection.class);
    ...
    TypedQuery<SettlementStatementRowProjection> typedQuery =
        entityManager.createQuery(contentQuery);
    typedQuery.setFirstResult((int) filter.pageable().getOffset());
    typedQuery.setMaxResults(filter.pageable().getPageSize());

    List<SettlementStatementRow> content =
        typedQuery.getResultList().stream().map(this::toStatementRow).toList();
  ...
  private List<Order> resolveOrders(
      Pageable pageable, CriteriaBuilder cb, Root<SettlementEntity> root) {
    List<Order> orders = new ArrayList<>();
    for (Sort.Order order : pageable.getSort()) {
      if ("settledAt".equals(order.getProperty())) {
        orders.add(
            order.isAscending() ? cb.asc(root.get("settledAt")) : cb.desc(root.get("settledAt")));
      }
