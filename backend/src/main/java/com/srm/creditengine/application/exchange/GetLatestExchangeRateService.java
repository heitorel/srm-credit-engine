...
public class GetLatestExchangeRateService {
    ...
    public ExchangeRateResult getLatest(String sourceCurrencyRaw, String targetCurrencyRaw) {
        CurrencyCode sourceCurrency = referenceDataService.requireSupportedCurrency(sourceCurrencyRaw, "sourceCurrency");
        CurrencyCode targetCurrency = referenceDataService.requireSupportedCurrency(targetCurrencyRaw, "targetCurrency");
        if (sourceCurrency.value().equals(targetCurrency.value())) {
            throw new InvalidCurrencyPairException();
        }

        return exchangeRateRepository.findFirstBySourceCurrency_CodeAndTargetCurrency_CodeOrderByValidAtDescCreatedAtDescIdDesc(
                        sourceCurrency.value(),
                        targetCurrency.value()
                )
                .map(ExchangeRateEntity::toResult)
                .orElseThrow(() -> new ExchangeRateNotFoundException(sourceCurrency.value(), targetCurrency.value()));
    }
}
