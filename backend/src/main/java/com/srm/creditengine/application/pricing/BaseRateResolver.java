...
public class BaseRateResolver {
  ...
  @Autowired
  public BaseRateResolver(
      @Value("${application.defaults.base-rate:#{null}}") String defaultBaseRateRaw) {
    this(
        defaultBaseRateRaw == null || defaultBaseRateRaw.isBlank()
            ? null
            : new BigDecimal(defaultBaseRateRaw));
  }
