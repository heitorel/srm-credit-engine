...
public class StartupTestDataService {
    ...
    private static final SeedExchangeRateDefinition BRL_USD_PREVIOUS = new SeedExchangeRateDefinition(
    ...
    private static final SeedExchangeRateDefinition BRL_USD_LATEST = new SeedExchangeRateDefinition(
    ...
    private static final SeedExchangeRateDefinition USD_BRL_PREVIOUS = new SeedExchangeRateDefinition(
    ...
    private static final SeedExchangeRateDefinition USD_BRL_LATEST = new SeedExchangeRateDefinition(
    ...
    private static final SeedAssignorDefinition LUMEN_ASSIGNOR = new SeedAssignorDefinition(
            "55555555-5555-5555-5555-555555555555",
            "Lumen Atacado Ltda.",
            "11222333000144"
    );
    private static final SeedAssignorDefinition ACME_ASSIGNOR = new SeedAssignorDefinition(
            "66666666-6666-6666-6666-666666666666",
            "ACME Comercio Ltda.",
            "12345678000199"
    );
    private static final SeedAssignorDefinition ORBIT_ASSIGNOR = new SeedAssignorDefinition(
            "77777777-7777-7777-7777-777777777777",
            "Orbit Foods S.A.",
            "99887766000155"
    );
    private static final SeedAssignorDefinition BLUE_ASSIGNOR = new SeedAssignorDefinition(
    ...
    private void seedExchangeRate(SeedExchangeRateDefinition definition) {
        ...
        CurrencyEntity sourceCurrency = currencyRepository.getReferenceById(definition.sourceCurrency());
        CurrencyEntity targetCurrency = currencyRepository.getReferenceById(definition.targetCurrency());
        ...
        exchangeRateRepository.save(ExchangeRateEntity.of(exchangeRate, sourceCurrency, targetCurrency));
    }
    ...
    private void seedAvailableReceivable(
        ...
        if (receivableRepository.findByAssignor_IdAndExternalReference(assignor.getId(), externalReference).isPresent()) {
            return;
        }

        ReceivableTypeEntity receivableType = receivableTypeRepository.getReferenceById(receivableTypeCode);
        CurrencyEntity currency = currencyRepository.getReferenceById(currencyCode);
        LocalDateTime now = utcNow();

        receivableRepository.save(ReceivableEntity.create(
                id,
                assignor,
                externalReference,
                receivableType,
    ...
    private void seedSettlements() {
        seedSettlement(new SeedSettlementDefinition(
                ACME_ASSIGNOR,
                "BRL",
                new BigDecimal("0.01000000"),
                List.of(
                        new SeedSettlementItemDefinition("NF-2001", "10000.00", "BRL", "MERCANTILE_DUPLICATE", 55),
                        new SeedSettlementItemDefinition("CHK-2002", "6500.00", "BRL", "POST_DATED_CHECK", 80)
                )
        ));
        seedSettlement(new SeedSettlementDefinition(
                ORBIT_ASSIGNOR,
                ...
                List.of(
                        new SeedSettlementItemDefinition("NF-3001", "22000.00", "BRL", "MERCANTILE_DUPLICATE", 50),
                        new SeedSettlementItemDefinition("NF-3002", "9500.00", "BRL", "POST_DATED_CHECK", 95)
                )
        ));
        seedSettlement(new SeedSettlementDefinition(
                BLUE_ASSIGNOR,
                ...
                List.of(
                        new SeedSettlementItemDefinition("INV-4001", "12500.00", "USD", "MERCANTILE_DUPLICATE", 70)
                )
        ));
    }

    private void seedSettlement(SeedSettlementDefinition definition) {
        Optional<AssignorEntity> assignor = assignorRepository.findFirstByDocument(definition.assignor().document());
        long existingItems = assignor.stream()
                .flatMap(entity -> definition.receivables().stream()
                        .map(item -> receivableRepository.findByAssignor_IdAndExternalReference(entity.getId(), item.externalReference()))
                )
        ...
        if (existingItems > 0) {
            throw new IllegalStateException(
                    "Startup settlement seed for assignor document %s is partially present.".formatted(definition.assignor().document())
            );
        }

        createSettlementService.create(new CreateSettlementRequest(
                ...
                definition.receivables().stream()
                        .map(item -> new ReceivableSettlementRequest(
    ...
    private AssignorEntity resolveAssignor(SeedAssignorDefinition definition) {
        return assignorRepository.findFirstByDocument(definition.document())
                .orElseGet(() -> {
                    LocalDateTime now = utcNow();
                    return assignorRepository.save(AssignorEntity.create(
                            definition.id(),
                            definition.name(),
                            definition.document(),
                            now,
    ...
    private record SeedAssignorDefinition(
            String id,
            String name,
            String document
    ) {
