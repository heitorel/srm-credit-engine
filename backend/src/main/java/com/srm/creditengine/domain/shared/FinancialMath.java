...
public class FinancialMath {
  ...
  public BigDecimal calculatePresentValue(
      BigDecimal faceValue, BigDecimal baseRate, BigDecimal spread, BigDecimal term) {
    BigDecimal denominator =
        pow(BigDecimal.ONE.add(baseRate, MATH_CONTEXT).add(spread, MATH_CONTEXT), term);
  ...
  private BigDecimal exp(BigDecimal value) {
    ...
    for (int i = 1; i < MAX_ITERATIONS; i++) {
      term = term.multiply(value, MATH_CONTEXT).divide(BigDecimal.valueOf(i), MATH_CONTEXT);
