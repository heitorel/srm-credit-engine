...
public class PricingEngine {
    ...
    public PricingResult price(PricingContext context) {
        ...
        var rawPresentValue = financialMath.calculatePresentValue(
                faceValue.amount(),
                context.baseRate().value(),
                spread.value(),
                context.term().value()
        );
        Money presentValue = new Money(rawPresentValue, faceValue.currency());
        Money netPaymentValue = currencyConversionService.convert(presentValue, context.paymentCurrency(), context.exchangeRate());
        ...
        return new PricingResult(
                ...
                context.exchangeRate() == null ? null : financialMath.roundRate(context.exchangeRate().value()),
